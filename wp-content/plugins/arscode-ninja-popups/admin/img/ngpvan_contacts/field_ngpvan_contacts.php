...
class SNP_NHP_Options_ngpvan_contacts extends SNP_NHP_Options
{
    ...
    public function render()
    {
        ...
        $this->field['options'] = snp_ml_get_ngpvan_contacts();
        ...
        if (!isset($this->field['meta'])) {
            echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button ngpvan_contacts_gl" name="" value="Grab Contact Codes" />';
        }
    ...
    {
        wp_enqueue_script(
            'nhp-opts-field-ngpvan_contacts-js', SNP_NHP_OPTIONS_URL . 'fields/ngpvan_contacts/field_ngpvan_contacts.js', array('jquery', 'farbtastic'), time(), true
        );
    }
}