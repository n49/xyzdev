...
class SNP_NHP_Options_mailfit_lists extends SNP_NHP_Options
{
    ...
    public function render()
    {
        ...
        $this->field['options'] = snp_ml_get_mailfit_lists();
        ...
        if (!isset($this->field['meta'])) {
            echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button mailfit_lists_gl" name="" value="Grab Lists" />';
        }
    ...
    {
        wp_enqueue_script(
            'nhp-opts-field-mailfit_lists-js', SNP_NHP_OPTIONS_URL . 'fields/mailfit_lists/field_mailfit_lists.js', array('jquery', 'farbtastic'), time(), true
        );
    }
}