...
$req = curl_init($url);
$postfields = [
	"client_id" => "1514214584.1654713347",
	"events" => 
		[
			"name" => $category
		]
...
function html5blank_header_scripts()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        ...
        wp_register_script('menuinit', get_template_directory_uri() . '/js/menuinit.js', array('jquery'), '1.0.0');
        wp_enqueue_script('menuinit');

        wp_register_script('waypoints', get_template_directory_uri() . '/js/jquery.waypoints.min.js', array('jquery'), '4.0.1', true);
        wp_enqueue_script('waypoints');

        wp_register_script('sticky', get_template_directory_uri() . '/js/sticky.min.js', array('jquery'), '4.0.1', true);
        wp_enqueue_script('sticky');

        wp_register_script('slick', get_template_directory_uri() . '/js/slick.min.js', array('jquery'), '1.8.1', true);
        wp_enqueue_script('slick');

        wp_register_script('lightgallery', get_template_directory_uri() . '/js/lightgallery.min.js', array('jquery'), '1.6.1', true);
        wp_enqueue_script('lightgallery');

        wp_register_script('tabslet', get_template_directory_uri() . '/js/jquery.tabslet.min.js', array('jquery'), '1.7.4', true);
        wp_enqueue_script('tabslet');

        wp_register_script('cookie', get_template_directory_uri() . '/js/js.cookie.js', array('jquery'), '2.2.0', true);
        wp_enqueue_script('cookie');

        wp_register_script('datepicker', get_template_directory_uri() . '/js/datepicker.min.js', array('jquery'), '2.2.3', true);
        wp_enqueue_script('datepicker');

		wp_register_script('flatpickr', get_template_directory_uri() . '/js/flatpickr.js', array('jquery'), '4.6.4', true);
        wp_enqueue_script('flatpickr');

		wp_register_script('chosen', get_template_directory_uri() . '/js/chosen.jquery.min.js', array('jquery'), '1.6.2', true);
		wp_enqueue_script('chosen');

        wp_register_script('simple-accordion', get_template_directory_uri() . '/js/simple.accordion.js', array('jquery'), '1.0.0', true);
        wp_enqueue_script('simple-accordion');

        wp_register_script('html5blankscripts', get_template_directory_uri() . '/js/scripts.js', array('jquery'), '1.0.6', true);
...
function html5blank_conditional_scripts()
{
	if (is_page_template('template-move-out.php')) {
        wp_register_script('move_out', get_template_directory_uri() . '/js/move_out.js', array('jquery'), '1.0.0', true);
        wp_enqueue_script('move_out');
    }
	
    if (is_page_template('template-about.php')) {
        wp_register_script('timeline', get_template_directory_uri() . '/js/timeline.js', array('jquery'), '1.0.0', true);
	...
	if (is_page_template('template-locations.php') ||  is_page_template('template-storage-sizes.php') || is_page_template('template-storage-2.php') || is_page_template('template-storage.php') || is_page_template('template-pricing.php') || is_page_template('template-contact.php') || is_page_template('template-storage-information.php')) {
        wp_register_script('googlemapapi', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyBkINJMV7xbxQzzefXijdLJKE7kGxsSgUo&extension=.js&libraries=geometry,places&v=3', array('jquery'));
        ...
        wp_register_script('locations', get_template_directory_uri() . '/js/locations.js', array('jquery'), '1.0.0', true);
	...
	if(is_page_template('template-locations-book.php')){
		wp_register_script('bookUnit', get_template_directory_uri() . '/js/bookUnit.js', array('jquery'), time(), true);
	...
	if(is_page_template('template-customer-details.php') && !is_page(19403)){
      	wp_register_script('customer_details', get_template_directory_uri() . '/js/customer_details.js', array('jquery'), '1.0.3', true);
        wp_enqueue_script('customer_details');
    }
    if(is_page(19403)){
      	wp_register_script('customer_details', get_template_directory_uri() . '/js/customer_details_pr.js', array('jquery'), '1.0.3', true);
        wp_enqueue_script('customer_details');
    }
    if(is_page(9048)){
      	wp_register_script('rent_now', get_template_directory_uri() . '/js/rent_now.js', array('jquery'), '1.0.1', true);
        wp_enqueue_script('rent_now');
    }
    if(is_page_template('template-rent-now.php')){
      wp_register_script('clipboard', get_template_directory_uri() . '/js/clipboard.min.js', array('jquery'), '2.0.11', true);
      wp_enqueue_script('clipboard');

      wp_register_script('rentform', get_template_directory_uri() . '/js/rentform.js', array('jquery'), '1.0.0', true);
      wp_enqueue_script('rentform');

       wp_register_script('rent_new', get_template_directory_uri() . '/js/rent_now_new.js', array('jquery'), '1.0.0', true);
      wp_enqueue_script('rent_new');
  }
	if(is_page_template('template-choose-your-space.php')){
		wp_register_script('chooseYourSpace', get_template_directory_uri() . '/js/chooseYourSpace.js', array('jquery'), '1.0.3', true);
...
function create_faq_taxonomies() {
	...
	$args = array(
		...
		'query_var'         => true,
		'rewrite'           => array( 'slug' => 'faq-category', 'hierarchical' => true ),
	);
...
function create_unit_taxonomies() {
	...
	$args = array(
		...
		'show_admin_column' => true,
		'query_var'         => true,
		'rewrite'           => array( 'slug' => 'unit-category', 'hierarchical' => true ),
	);

	register_taxonomy('unit-category', array( 'units'), $args);
}
...
function ssm_iframe_url(){
	if(
		(!isset($_COOKIE['unitRent']) || !isset($_COOKIE['unitLocation']) || !isset($_COOKIE['unitType']) || !isset($_COOKIE['length']) || !isset($_COOKIE['width']))
	){
	  ...
	  $URL = "https://e-storageonline.com/xyzstorage/Contents/RentOnline_TD.aspx".
			  "?FromPage=LocationDetails&ReqPage=ALL".
			  "&LocId=".$L_ID."&UnitId=".$U_ID."&Len=".$LENGTH."&Bre=".$BREADTH."&UnitRent=".$RENT;
...
curl_setopt_array($curl, [
  CURLOPT_URL => "http://ssm20.selfstoragemanager.net/XYZStorageAPI/service.asmx/GetInsuranceSchemes?strCustomerCode=xyzstorage&strCustomerPassword=191820120406&strLocationCode=".$locationId,
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,
...
function ssm_reserve_api($data){
  ...
  $ssm_url =  "https://www.secure.selfstoragemanager.com/ssmwebserviceV2.1/ssmws.asmx/SaveReservationDetails_WEB?strCustomerCode=xyzstorage&strCustomerPassword=991852130418".
              "&strLocationCode=".$data['location'].
              "&strReservationDateTime=".$url_date.
              "&strDateRequired=".$url_date.
              "&strUnitTypeId=".$data['unit'].
              ...
              "&strStateCode=0".
              "&strZipCode=0".
              "&strEMail=".urldecode($data['email']).
              "&strHomePhone=".$data['phone'].
              "&strBusinessPhone=".$data['phone'].
              "&strCellPhone=".$data['phone'].
              "&strFaxNumber=".$data['phone'].
              "&strDiscountIds=0&strMarketingInfo=0".
              "&strCustomerNotes=".$quantity.curl_escape($curl,$details).
              "&strChkReservThresholdLimit=0".
              "&strApplyInternetPrice=0".
              "&strRequestOrigin=W&intUserId=1".
              "&UnitTypeRent=".$data['rent'].
	  		   "&strCompanyName=".$data['businessname'];
	curl_setopt_array($curl, array(
	  CURLOPT_URL => $ssm_url,
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_MAXREDIRS => 10,
...
// 	$details = urlencode($details);
  ...
  $ssm_url =  "https://www.secure.selfstoragemanager.com/ssmwebserviceV2.1/ssmws.asmx/SaveReservationDetails_WEB?strCustomerCode=xyzstorage&strCustomerPassword=991852130418".
              ...
              "&strCellPhone=".$data['phone'].
              "&strFaxNumber=".$data['phone'].
              "&strCompanyName=acss&strDiscountIds=0&strMarketingInfo=0".
              "&strCustomerNotes=".$quantity.curl_escape($curl,$details).
              "&strChkReservThresholdLimit=0".
              "&strApplyInternetPrice=0".
              "&strRequestOrigin=W&intUserId=1".
...
function opio_reviews($data){
  ...
  $ARGS = '?scopes=Review&subSkip=0&subLimit=25';
    ...
    CURLOPT_CONNECTTIMEOUT => 20
      ...
      $myObj = array( "rating" => $json[0]['aggregateRating']['5734f48a0b64d7382829fdf7']["average"]);
      return $myObj;
		}
	  $video_review = array();
	...
	if($data['object'] == 'both'){
		$myObj = array( 'rating' =>  $json[0]['aggregateRating']['5734f48a0b64d7382829fdf7']["average"] , 'video' => $video_review , "url" => $BASE_URL.$this_location_opio.$ARGS);
		return $myObj;
	}
	else return false;
}
...
function video_review($data){
  ...
  $BASE_URL = 'https://op.io/api/entities/video-review/';
    ...
    $response = wp_remote_get(
		$BASE_URL.$this_location_opio,
		array("headers" => $API_KEY, "timeout" => 20)
	);
	...
	return array(
		...
		'video' => $video_review , "url" => $BASE_URL.$this_location_opio
	);
}
...
function unit_discounts($data){
  $curl = curl_init();
  $url = "https://www.secure.selfstoragemanager.com/ssmwebserviceV2.1/ssmws.asmx/GetUnitTypeDiscounts".
          "?strCustomerCode=xyzstorage".
          "&strCustomerPassword=991852130418".
          "&strLocationCode=".$data['location'].
          "&strUnitTypeId=".$data['id'];
  curl_setopt_array($curl, array(
    CURLOPT_URL => $url,
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_MAXREDIRS => 10,
  ...
  if($api_unit_details['Discount']['DiscountType']) return array($api_unit_details['Discount']);
...
function unit_details($data){
  ...
  $url = "https://www.secure.selfstoragemanager.com/ssmwebserviceV2.1/ssmws.asmx/GetUnitTypeDetailsForGivenUnitType".
          "?strCustomerCode=xyzstorage&strCustomerPassword=991852130418".
          "&strLocationCode=".$data['location'].
          '&strLength='.$data['length'].
          '&strWidth='.$data['width'].
          "&strUnitTypeId=".$data['unitid'].
          '&strAlreadySelectedUnitTypes=0&strReservationThresholdValue=0'.
          '&UnitTypeRent='.$f_unit['rent'];
...
function debugme($data){
	...
	$return = array("post" => $post, "meta" => $post_meta);
	return $return;
}
...
function opio_reviews_home($data){
	...
	$response = wp_remote_get($url, array("headers" => $API_KEY, "timeout" => 5));
...
function get_unit_type_prices($location){
	...
	foreach($response as $unit) {
		...
		if (!$unit_type_prices[$type_id]) {
			...
			$unit_type_prices[$type_id] = array( $level_price => 1 );
